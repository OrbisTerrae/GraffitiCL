...
long getMDTimeScale() {
    return md.time_scale;
}
long getMDModificationTime() {
    return md.modification_time;
}
long getMDCreationTime() {
    return md.creation_time;
}
...
double getGPMF_videolength() {
    return GPMF_videolength;
}

uint32_t getMetasize_count() {
    return metasize_count;
}
...
long getTrakClockdemon(){
    return trak_clockdemon;
}
long getTrakClockcount(){
    return trak_clockcount;
}
long getMetaClockdemon(){
    return meta_clockdemon;
}
long getMetaClockcount(){
    return meta_clockcount;
}
    ...
    if (fp) {
        ...
        do {
            ...
            if (len == 8) {
                ...
                if (qttag == MAKEID('H', 'M', 'M', 'T')) {
                    int hl;
                    int hlms;

                    if (debug) printf("HMMT: GOPRO MANUAL TAGS\n");
                    len = fread(&qtsize32, 1, 4, fp); // not the way it's supposed to be used
                    hl = BYTESWAP32(qtsize32);
                    hmmt->nb = hl;
//                    if (hl > 0) if (verbose) printf("%s\n", basename(filename));
                    for (int i = 0; i < hl; i++) {
                        len = fread(&qtsize32, 1, 4, fp); // not the way it's supposed to be used
                        hlms = BYTESWAP32(qtsize32);
                        hmmt->samples[i] = hlms;
                        if (debug) printf("[%d] %7dms \n", i, hlms);
                    }
                    return metadatalength;
                }
    ...
    if (fp) {
        ...
        do {
            ...
            if (len == 8) {
                ...
                if (qttag == MAKEID('F', 'I', 'R', 'M')) {
                    if (debug)
                        printf("FIRM: GOPRO NAME + FW VERSION \n");

                    char str1[32];

                    len = fread(&str1, 1, 4, fp); // not the way it's supposed to be used

                    //hl = BYTESWAP32(qtsize32);
                    for (int i = 0; i < 32; i++) {
                        if (debug) printf("'%c'", str1[i]);
                    }
                    if (debug) printf("\n");
    ...
    if (fp) {
        ...
        do {
            ...
            if (len == 8) {
                ...
                if (qttag == MAKEID('V', 'I', 'D', 'E')) {
                    if (debug) printf("VIDE: resolution + frame rate \n");
                }
                if (qttag == MAKEID('F', 'I', 'R', 'M')) {
                    if (debug) printf("FIRM: GOPRO NAME + FW VERSION \n");
                }
                if (qttag == MAKEID('S', 'E', 'T', 'T')) {
                    if (debug) printf("SETT: GOPRO SETTINGS \n");
                }
    ...
    if (fp) {
        ...
        do {
            ...
            if (len == 8) {
                ...
                uint32_t sett1 = -1, sett2 = -1, sett3 = -1;
                uint32_t set1 = -1, set2 = -1, set3 = -1;

                if (qttag == MAKEID('S', 'E', 'T', 'T')) {

                    if (debug) printf("SETT: GOPRO SETTINGS \n");
                    len = fread(&sett1, 1, 4, fp);
                    set1 = BYTESWAP32(sett1);
                    sett->set1 = set1;

                    len = fread(&sett2, 1, 4, fp);
                    set2 = BYTESWAP32(sett2);
                    sett->set2 = set2;

                    len = fread(&sett3, 1, 4, fp);
                    set3 = BYTESWAP32(sett3);
                    sett->set3 = set3;
    ...
    if (fp) {
        ...
        do {
            ...
            if (len == 8) {
                ...
                {
/*gb: To get the file duration, you will need to find the moov atom of the file, and inside it the mvhd atom. The duration, in seconds, can be calculated by dividing the mvhd's Duration field by the TimeScale field. */
                    len = fread(&skip, 1, 4, fp);
                    len += fread(&skip, 1, 4, fp);
                    len += fread(&skip, 1, 4, fp);
                    len += fread(&clockdemon, 1, 4, fp);
                    clockdemon = BYTESWAP32(clockdemon);
                    len += fread(&clockcount, 1, 4, fp);
                    clockcount = BYTESWAP32(clockcount);
                ...
                } else if (qttag == MAKEID('m', 'd', 'h', 'd')) //mdhd  media header
                {
                    ...
                    if (len == sizeof (md)) {
                        ...
                        if (GPMF_videolength == 0.0) // Get the video length from the first track
                        {
                            GPMF_videolength = (double) trak_clockcount / (double) trak_clockdemon;
                        }
                    }
                    LONGSEEK(fp, qtsize - 8 - len, SEEK_CUR); // skip over mvhd
...
cleanup:
    if (payload) if (payload) FreeGPMFPayload(payload);
    payload = NULL;
